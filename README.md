...
- `hetzner-sno-prepare-pxe.sh [--disk-device <device_path>] <ocp_version> <pull_secret_file> <base_domain> [cluster_name] [rendezvous_ip]`: Designed to be run in the Hetzner Rescue System **before** `hetzner-sno-provision-host-agentbased.sh`. Because the rescue environment is based on Debian 12, it installs `nmstatectl` (required by `openshift-install` to validate NMState YAML) via Rust/Cargo instead of the package manager. It then downloads version-matched `oc` and `openshift-install` binaries with checksum validation, inspects the machine's network configuration, autodetects the install disk unless `--disk-device` is provided, generates `install-config.yaml` and `agent-config.yaml`, runs `openshift-install agent create pxe-files`, and copies the resulting boot artifacts to `/root`.
