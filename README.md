...
- `hetzner-sno-provision-host.sh [options] <iPXE script URL>`: It is designed to run run from Hetzner Rescue System. You have to provide the iPXE URL generated by assisted installer in the `<iPXE script URL>` parameter, then the script downloads the iPXE script, validates that it contains kernel and initrd entries, downloads the artifacts and uses them to install OpenShift.
